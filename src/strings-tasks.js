...
function getStringLength(value) {
  return typeof value === 'string' ? Number(value.length) : 0;
}
...
function concatenateStrings(value1, value2) {
  return value1.trimStart().concat(value2.trimEnd());
}
...
function removeFirstOccurrences(str, value) {
  ...
  return startIndex === -1
    ? str
    : str.slice(0, startIndex) + str.slice(endIndex);
}
...
function countVowels(str) {
  ...
  const count = Object.is(str.match(/[aeiouy]/gi), null)
    ? 0
...
function isPalindrome(str) {
  const formatStr = str
    ...
    .join('')
    .toLowerCase();
  const newStr = formatStr.split('').reverse().join('');
...
function extractNameFromTemplate(value) {
  ...
  return value.replace(/Hello, (\w+) (\w+)!/, (match, firstName, LastName) => {
...
function getCardId(value) {
  ...
  return cardsDeck.replace(/[']/g, '').split(',').indexOf(value);
}
